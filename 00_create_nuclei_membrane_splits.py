...
@hydra.main(
    version_base="1.3", config_path="conf", config_name="scenario_segment_star_cellpose"
)
def main(config: VollCellSegPose):
    ...
    # Support configurable input filename (defaults to 'Merged.tif')
    input_filename = getattr(
        config.experiment_data_paths, "input_merged_filename", "Merged.tif"
    )
    merged_path = os.path.join(parent_directory, input_filename)
    print(f"Reading merged data from: {merged_path}")
    ...
    imwrite(
        os.path.join(nuclei_image_dir, f"{timelapse_nuclei_to_track}.tif"),
        nuclei_data,
        imagej=True,
    ...
    imwrite(
        os.path.join(membrane_image_dir, f"{timelapse_nuclei_to_track}.tif"),
        membrane_data,
        imagej=True,
        photometric="minisblack",
        resolution=(1 / voxel_size_xyz[0], 1 / voxel_size_xyz[1]),
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for time_point in range(time_points):
            ...
            futures.append(
                executor.submit(save_image, image_data, data_name, save_dir, time_point)
            )
